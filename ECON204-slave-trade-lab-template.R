...
## 4.4 Replicate the regression from 4.1, but add controls for being an island and being in the mena region (just list the formula as y ~ x1 + x2 + x3 in lm). Your results should replicate the regression on Slide 20 in Lecture 3.  What is the coefficient associated with ln_slave_exports in this regression?
...
## 6. The code below makes a scatter plot of the relationship between log GDP in 2010 and the log number of slaves exported, normalized by land area. Adapt the code to make a scatterplot of the relationship between infant mortality and log slave exports.
...
## d. You can overlay a scatter plot and a linear regression line by adding an additional layer (or "geom"), as in the code for scatter plots of infant mortality and GDP below. Add a linear regression line to your scatter plot of infant mortality and log slave exports. 
